...
const clientId = "639199898177-doqpqg5380rd5i69s6ijjma1hh5n6o53.apps.googleusercontent.com";
...
function Signup() {
    ...
    const validateEmail = (email) => {
        ...
        if (!emailRegex.test(email)) {
            setEmailError("Invalid email format. Please use a valid @gmail.com email.");
            return false;
        }
        setEmailError("");
        return true;
    ...
    const handlesubmit = async (e) => {
        ...
        if (!validateEmail(newUser.email)) {
            toast.warn("Invalid email format. Please use a valid @gmail.com email.");
            return;
        }

        const isUsernameTaken = fetchdata.some(user => user.username === newUser.username);
        const isEmailTaken = fetchdata.some(user => user.email === newUser.email);
